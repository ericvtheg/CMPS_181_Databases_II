...
RC RecordBasedFileManager::readRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid, void *data) {
   ...
   SlotDirectoryHeader header = getSlotDirectoryHeader(orPageData);
   ...
   // }
   if(compRID(rid, new_rid) && (recordEntry.offset == 0) && (recordEntry.length == 0)){
       //cout << " Same RID " << endl;
       free(orPageData);
       free(pageData);
       return RBFM_SLOT_DN_EXIST;
       //We've hit a dead chain
   }else if(!compRID(rid, new_rid) && (recordEntry.offset == 0 &&  (recordEntry.length == 0))){
       ...
       //Set the original slot to finally be dead
...
RID RecordBasedFileManager::findRID(FileHandle &fileHandle, void *pageData, const RID &rid){
   ...
   int32_t new_pageNum = 0;
   new_pageNum = rid.pageNum;
   int32_t new_slotNum = 0;
   new_slotNum = rid.slotNum;
   int32_t prev_slotNum = -1;
   int32_t prev_pageNum = 0;
   bool dbl_jump = false;
   RID new_rid;
   ...
   while(record.offset < 0){
       ...
       new_pageNum = (record.offset* -1) - 1;
       new_slotNum = record.length;
       if(dbl_jump){

           first_rec.offset = (int32_t) record.offset;
...
RC RecordBasedFileManager::updateRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, const RID &rid){
   ...
   if(fileHandle.readPage(found_rid.pageNum, pageData))
               return RBFM_READ_FAILED;
   ...
   if(record.length == to_load_recordSize){
       ...
       //coalesce
       unsigned to_shift_data_size =  record.offset - header.freeSpaceOffset;
       void * records_to_load = malloc(to_shift_data_size);
       ...
       if(found_rid.pageNum == i){ // if page found is same page as original record
           ...
           for(i = 0; i < header.recordEntriesNumber; i++){
               ...
               if(i == found_rid.slotNum){
                   // kill rid
                   // add bleow line by 1 for newly added slot
                   upd_entry.length = headerV2.recordEntriesNumber+1;
                   upd_entry.offset = (int32_t) (inserted_pageNum+1) * -1;
               }else if(record.offset > upd_entry.offset){
                   upd_entry.offset += record.length;
               }else{
                   continue;
